...
# arguments_strModel = 'chairs-final'
...
if __name__ == '__main__':
	...
	with torch.no_grad():
		tenOutput = estimate(tenSecond, tenFirst)  # backward flow

	w00 = backwarp(tenFirst[None].cuda(), tenOutput[None].cuda())
	# w0 = warping(tenFirst[None].cuda(), tenOutput[None].cuda())
	noc_mask2 = torch.exp(-50 * torch.sum(tenSecond[None].cuda() - w00, dim=1).pow(2)).unsqueeze(1)
