...
def main(arg):
    ...
    models_list.append([osp.join(root_dir, 'models/SPADE_E2VID_ABS.pth'), 2])
    ...
    for i in tqdm.tqdm(vals):
        ...
        for d, frm in tqdm.tqdm(data_dir):
            te = testset(d, ev_rate, norm_e=norm, e_abs=True, num_freams=frm)
            ...
            # print(d.split('/')[-1])
    df_full['mse'].to_csv(osp.join(root_dir, 'res/test_full_mse_abs.csv'))
    df_full['ssim'].to_csv(osp.join(root_dir, 'res/test_full_ssim_abs.csv'))
    df_full['lpips'].to_csv(osp.join(root_dir, 'res/test_full_lpips_abs.csv'))
    df_full['tc'].to_csv(osp.join(root_dir, 'res/test_full_tc_abs.csv'))

    df_10['mse'].to_csv(osp.join(root_dir,  'res/test_10_mse_abs.csv'))
    df_10['ssim'].to_csv(osp.join(root_dir, 'res/test_10_ssim_abs.csv'))
    df_10['lpips'].to_csv(osp.join(root_dir, 'res/test_10_lpips_abs.csv'))
    df_10['tc'].to_csv(osp.join(root_dir, 'res/test_10_tc_abs.csv'))

    torch.save(data_dict, osp.join(root_dir, '/res/full_data_abs.pth'))
